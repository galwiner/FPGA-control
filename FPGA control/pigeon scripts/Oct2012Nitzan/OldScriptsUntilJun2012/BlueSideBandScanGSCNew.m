...
%         % continuous GSC 
%         prog.GenSeq([Pulse('674DDS1Switch',1,dic.vibMode(mode).coolingTime,'freq',Freq674SinglePass+dic.vibMode(mode).freq),...
...
%                      Pulse('OpticalPumping',0,dic.vibMode(mode).coolingTime+dic.T1033+dic.Toptpump)]);        
        ...
        if (~isempty(Mode2Cool))
            ...
            prog.GenRepeatSeq([Pulse('NoiseEater674',2,dic.vibMode(mode).coldPiTime),...
                               ...
                               Pulse('OpticalPumping',dic.vibMode(mode).coldPiTime+dic.T1033,dic.Toptpump)],6);                          
             end
        end
