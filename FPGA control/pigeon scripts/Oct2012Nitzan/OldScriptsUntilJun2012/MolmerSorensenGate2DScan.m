...
GateTime=[1:10:301];  
% GateDetuning=0.005:0.0005:0.03;
GateDetuning=0.0:0.0005:0.03;

repetitions=50;
DifferentialDetuning=0.00;
novatechpower1=80*1.5;
novatechpower2=72*1.5;
...
if dic.SitOnItFlag
    ...
    while (cont)
        ...
        dic.setNovatech4Amp(2,1000);dic.setNovatech4Freq(2,dic.F674+SBoffset/2);
    ...
    for modeInd=1:length(Vmodes)
        ...
        for index1=1:length(GateDetuning)
            ...
            LightShift=0;%*-0.0049;
            dic.setNovatech4Amp(2,1000);dic.setNovatech4Freq(2,dic.updateF674+SBoffset/2);
            dic.setNovatech4Freq(1,Freq674SinglePass-(dic.vibMode(Vmodes(modeInd)).freq-GateDetuning(index1)));
            dic.setNovatech4Freq(0,Freq674SinglePass+(dic.vibMode(Vmodes(modeInd)).freq-GateDetuning(index1)+DifferentialDetuning));
            ...
            dic.GUI.sca(4);
            imagesc(GateTime,GateDetuning,abs(p2-p0)+p1);
            axis([min(GateTime) max(GateTime) min(GateDetuning) max(GateDetuning)]);
            colorbar;
            ylabel('GateDetuning( kHz)'); xlabel('GateTime(mus)'); title('|P2-P0|+P1');
        end
    end
end
%------------ Save data ------------------
