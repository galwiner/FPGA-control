function FakeBlueSideBandScanGSCNew(Vmodes)
    ...
    for modeInd=1:length(Vmodes)
        ...
        for index1 = 1:length(PulseTime)
            for lobeIndex = 1:2
                ...
                LightShift=-0.0049;
                dic.setNovatech4Amp(0,1000);dic.setNovatech4Freq(0,dic.F674);
                dic.setNovatech4Freq(1,Freq674SinglePass-(dic.vibMode(Vmodes(modeInd)).freq+LightShift));
                dic.setNovatech4Freq(2,Freq674SinglePass+(dic.vibMode(Vmodes(modeInd)).freq+LightShift));
                %             set(darkCountLine(lobeIndex),'XData',[],'YData',[]);
                
                r=experimentSequence(lobeIndex*2-3,PulseTime(index1),Vmodes(modeInd));
                dic.GUI.sca(1); %get an axis from Dictator GUI to show data
                hist(r,0:1:(1.8*dic.maxPhotonsNumPerReadout));
                if dic.TwoIonFlag
...
%--------------------------------------------------------------------
    function r=experimentSequence(redorblue,pulsetime,mode)
        prog=CodeGenerator;
        ...
        prog.GenSeq([Pulse('NoiseEater674',2,pulsetime),...
                     Pulse('674DDS1Switch',2,pulsetime,'freq',Freq674SinglePass+redorblue*dic.vibMode(mode).freq)]);
        % detection
