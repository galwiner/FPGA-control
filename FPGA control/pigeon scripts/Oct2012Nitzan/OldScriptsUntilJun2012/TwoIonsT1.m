...
if ~exist('waittime')
     waittime=[100 1000 5000 10000 20000]; %in msec
end
...
darkdown=zeros(iterationsize,length(waittime));
...
for index2=sind2:iterationsize
    ...
    for index1 = 1:length(waittime)
        ...
        %%%%%%%%%%%%%%%%%%%%%
        % Measure Spin Up (loop measurement if crystal melted at the end)
        ...
        % Measure Spin Down (loop measurement if crystal melted at the end)
        valid=0; limitcounter=5;
        while ~valid&&limitcounter
        ...
        %%%%%%%%%%%%%%%%        
        if dic.stp || ~final ||(lengthr~=chunksize)||(lengthrdown~=chunksize)||~limitcounter
            savethis;
            return;
        end
...
%                                  )/2/lengthr*100;
 ...
 %           AddLinePoint(lines(3),waittime(index1),mean((1-initialCrystal(1:index2,index1))*100));
...
%--------------- Save data ------------------
    ...
    function savethis
        ...
        save(saveFileName,'progress','waittime','dark','darkdown','initialCrystal','finalCrystal','chunksize','repetitions','index1','index2','Vkeith','Vcap','showData','dicParameters','scriptText');
        disp(['Save data in : ' saveFileName]);
        
    end

%--------------------------------------------------------------------
    function r=experimentSequence(waitt,freq,measDown)
        prog=CodeGenerator;
