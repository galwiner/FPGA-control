...
%             RFResScanRamsey;
            ...
            dic.setNovatech('Echo','freq',dic.SinglePass674freq-SBoffset,'amp',1000,'phase',dic.AddrInfo.P674Echo+3*pi/2);
            ...
            elseif isequal(matrixindex,[1 2])  %%% CHECKED
                r=experimentSequence(Gy,0,0,2,GateTime(indext));CountStat=1;
            elseif isequal(matrixindex,[1 3]) %CHECKED%
                r=experimentSequence(Gmx,0,0,2,GateTime(indext));CountStat=1;
            elseif isequal(matrixindex,[1 4]) %% CHECKED
                r=experimentSequence(0,0,0,2,GateTime(indext));CountStat=1;
                
            elseif isequal(matrixindex,[2 1]) %% CHECKED %
                r=experimentSequence(Gy,0,0,1,GateTime(indext));CountStat=1;
            elseif isequal(matrixindex,[3 1]) %% CHECKED
                r=experimentSequence(Gmx,0,0,1,GateTime(indext));CountStat=1;
            elseif isequal(matrixindex,[4 1]) %% CHECKED
                r=experimentSequence(0,0,0,1,GateTime(indext));CountStat=1;
                
            elseif isequal(matrixindex,[2 2])  % CHECKED
                r=experimentSequence(Gy,0,0,0,GateTime(indext));CountStat=0;
            elseif isequal(matrixindex,[3 3]) % CHECKED
                r=experimentSequence(Gmx,0,0,0,GateTime(indext));CountStat=0;
            elseif isequal(matrixindex,[4 4])
                r=experimentSequence(0,0,0,0,GateTime(indext));CountStat=0;
                
            elseif isequal(matrixindex,[2 3]) %% CHECKED %%
                r=experimentSequence(0,Lmz,Gmx,0,GateTime(indext));CountStat=0;
            elseif isequal(matrixindex,[3 2])%% CHECKED %%
                r=experimentSequence(0,Lz,Gy,0,GateTime(indext));CountStat=0;
            elseif isequal(matrixindex,[2 4]) %%% CHECKED?
                r=experimentSequence(Gx,Lmz,Gmx,0,GateTime(indext));CountStat=0;
            elseif isequal(matrixindex,[4 2]) %% CHECKED
                r=experimentSequence(Gx,Lz,Gy,0,GateTime(indext));CountStat=0;
            elseif isequal(matrixindex,[3 4]) %%   CHECKED
                r=experimentSequence(Gmy,Lz,Gy,0,GateTime(indext));CountStat=0;
            elseif isequal(matrixindex,[4 3]) % CHECKED
                r=experimentSequence(Gmy,Lmz,Gmx,0,GateTime(indext));CountStat=0;
            end
            
            if ((index1==1)&(index2==1))
                spin(index1,index2,indext)=1; % Trace constraint on the density matrix
            else
                dic.GUI.sca(1); %get an axis from Dictator GUI to show data
                hist(r,0:1:(1.8*dic.maxPhotonsNumPerReadout));
                ivec=dic.IonThresholds;
                histograms(index1,index2,indext,:)=r;
                ...
                dark(index1,index2,indext)=tmpdark;
                darktoplot=dark(:,:,indext);
                
                % Counting according to the CountStat
                switch CountStat
                    case 0
                        % regular two ion statistics
                        spin(index1,index2,indext)=(-1*sum((r<dic.IonThresholds(2))&r>dic.IonThresholds(1))+1*sum(r>dic.IonThresholds(2))+1*sum(r<dic.IonThresholds(1)))/length(r);
                    case 1
                        % one is hidden.
...
%         ylabel('x'); xlabel('y'); title('Dark Counts');
    ...
    %--------------------------------------------------------------------
    function r=experimentSequence(g1,lz,g2,shelvingstyle,gatetime)
        prog=CodeGenerator;
        ...
        %%%% Gate %%%
        if doGate
            prog.GenSeq([Pulse('674Gate',1,gatetime),Pulse('674DoublePass',0,gatetime+1)]);
        end
