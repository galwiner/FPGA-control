...
RSB=0.8742;
BSB=-RSB;
scanVector=(-1:0.20:1); %red side band scan
 f674List = dic.F674+scanVector;
%f674List = 80:0.025:81;
pulseTime=dic.T674*25;
pulseAmp=100;
...
%%------------------------ experiment sequence -----------------
    function r=experimentSequence(pFreq,pTime,pAmp)
        ...
        prog.GenSeq(Pulse('674DDS1Switch',2,-1,'freq',pFreq+RSB,'amp',pAmp));
        ...
        % peform ground state cooling with 1033
        coolingTime=10*pTime;
        ...
        prog.GenSeq(Pulse('OpticalPumping',1,dic.Toptpump));
        % BSB
        prog.GenSeq(Pulse('674DDS1Switch',2,-1,'freq',dic.F674+BSB,'amp',pAmp));
        prog.GenSeq([Pulse('NoiseEater674',3,pTime) Pulse('674DDS1Switch',2,pTime)]);
         % detection
