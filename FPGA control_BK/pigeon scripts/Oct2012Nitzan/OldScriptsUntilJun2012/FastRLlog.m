function valid=FastRFlog(varargin)
dic=Dictator.me;
savedata=1;
...
dark = zeros(length(timeList),2);
    ...
    for index1 = 1:length(timeList)
        ...
        pause(0.1);
        r=experimentSequence(dic.FRF-1.25e-3,pulseTime,pulseAmp);
        dic.GUI.sca(1);
        ...
        freqShift=0;
        r=experimentSequence(dic.FRF-1.75e-3+freqShift,pulseTime,pulseAmp);
        dic.GUI.sca(1);
        hist(r,1:1:dic.maxPhotonsNumPerReadout);
        if dic.TwoIonFlag
...
%%------------------------ experiment sequence -----------------
    function r=experimentSequence(pFreq,pTime,pAmp)
        ...
        prog.GenSeq(Pulse('674DDS1Switch',2,-1,'freq',FreqSinglePass,'amp',100));
