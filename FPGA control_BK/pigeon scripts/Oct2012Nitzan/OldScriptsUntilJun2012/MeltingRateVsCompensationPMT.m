function MeltingRateVsCompensationPMT
dic=Dictator.me;

Amp=3.1; minAmp=2.1;
Vcap=400;
%  Vrfgrid=[2.5:0.03:3.1];

 Vdclg=[-2.6:0.1:-2.3];
% Vdclg=[-2.6 -2.4];
Vcomp=-6.25;

waittime=40;% in seconds
repetitions=20;
...
lines2 =InitializeAxes (dic.GUI.sca(8),...
    'Vdcl','EscapeTime','Rock n Roll',...
    [Vdclg(1) Vdclg(end)],[0 1.4*waittime],1);
...
% -------- Main function scan loops ------
 ...
 CrystalCheckPMT(Amp);
 escapetime=zeros(repetitions,length(Vdclg));
 for index2=1:length(Vdclg)
    dic.com.UpdateTrapElectrode(0,0,0,Vdclg(index2),Vcomp); pause(1);
...
%               disp('F');
        ...
        disp(sprintf('Vdcl=%1.2f [V] (rep:%2.f) // Escape Time = %4.2f',Vdclg(index2),index1,escapetime(index1,index2)));
...
%         set(lines(1),'XData',timeAxis,'YData',photonCount(index1,:));
    ...
    AddLinePoint(lines2(1),Vdclg(index2),escapemean);
    disp(sprintf('Vdcl=%1.2f [V]  Escape Time = %4.2f',Vdclg(index2),escapemean));
 end
...
%--------------- Save data ------------------
    function savethis(doprint)
        if (dic.AutoSaveFlag)
            ...
            showData='figure;plot(Vdclg,mean(escapetime));xlabel(''Vdcl [V]'');ylabel(''Escape Time'');';
            dicParameters=dic.getParameters;
            save(saveFileName,'Vdclg','escapetime','waittime','timeAxis','loop','samplingTime','photonCount','index1','showData','dicParameters','scriptText');
            if doprint
