function Spectrum674ScanNew(varargin)
% determine file name
dic=Dictator.me; 
destDir=dic.saveDir;
...
%   detuneList=[-2.2:0.02:-0.8];

% detuneList=[-2.5:0.03:-1.6];

  %  detuneList=[-2.2:0.005:-1.5];
...
for ind1=1:length(timeList)
    ...
    for ind2=1:length(detuneList)
        ...
        r=experimentSequence(timeList(ind1),Freq674SinglePass+detuneList(ind2));
...
%----------------------------------------------------------------
    function r=experimentSequence(pulseTime,pulseDetune)
        ...
        prog.GenSeq(Pulse('674DDS1Switch',0,-1,'amp',50));
        % Doppler coolng
        ...
        prog.GenSeq([Pulse('NoiseEater674',1,pulseTime-2),...
                     Pulse('674DDS1Switch',0,pulseTime,'freq',pulseDetune)]);
