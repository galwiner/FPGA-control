...
% main variables
%waitTime=[100 200 500 linspace(1000,10000,10)]; %in milli-seconds
waitTime=[100 200 500]; %in milli-seconds
secondPulseTimes=secondPulseTime;%*[0.1 1:5];
ExpRepetitions=50;
ExpChunkSize=25;
...
if ~silent
    ...
    lines =InitializeAxes (dic.GUI.sca(9),'wait time (ms)','Dark Counts %','Shelving prob',...
        [waitTime(1) waitTime(end)],[0 100],2);
...
%         disp(sprintf('P_up=%.2f ',mean(mean(reshape(out(1:iteration,:,1,1,1),ExpChunkSize*iteration,[])<8)*100)));
%         disp(sprintf('P_down=%.2f ',mean(mean(reshape(out(1:iteration,:,2,1,1),ExpChunkSize*iteration,[])<8)*100)));
    end
end
%------------------------ experiment sequence -----------------
