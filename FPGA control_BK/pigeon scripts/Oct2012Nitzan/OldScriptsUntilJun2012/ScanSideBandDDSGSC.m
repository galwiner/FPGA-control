function ScanSideBandDDSGSC(Vmodes)
...
%             set(darkCountLine(lobeIndex),'XData',[],'YData',[]);
            ...
            r=experimentSequence(Vmodes(modeInd),f674Span(index1),lobeIndex*2-3);
...
%--------------------------------------------------------------------
    function r=experimentSequence(mode,freqShift,sb)
        prog=CodeGenerator;
...
%         prog.GenSeq(Pulse('DIO7',0,1));
        ...
        prog.GenSeq([Pulse('NoiseEater674',2,dic.vibMode(mode).coldPiTime),...
                     Pulse('674DDS1Switch',2,dic.vibMode(mode).coldPiTime,'freq',Freq674SinglePass+sb*dic.vibMode(mode).freq+freqShift)]);
        % detection
