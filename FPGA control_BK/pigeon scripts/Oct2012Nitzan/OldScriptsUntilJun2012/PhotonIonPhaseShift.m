...
for ind1 =1:length(PulsePhase)
    ...
    ion=1+(r(3:4:end)>dic.darkCountThreshold);
    Ref(ind1)=1-mean(r(4:4:end)>dic.darkCountThreshold);
    expRep(ind1)=mean(r(2:4:end))*20;
    DensityM=zeros(2,2);
    ...
    AddLinePoint(lines(3),PulsePhase(ind1),Ref(ind1));
...
if (dic.AutoSaveFlag)
    ...
    showData=['figure;plot(PulsePhase,condProb(1,:),''-ob'',PulsePhase,condProb(2,:),''-xr'',PulsePhase,Ref,''-xr''); axis([0 6.3 0 1]);'...
              'xlabel(''photon phase'');ylabel(''cond. prob. upH and upV'');'...
              'figure;plot(PulsePhase,expRep);'];
    saveFileName=fullfile(destDir ,[fileName datestr(now,'-ddmmmyy-HHMMSS')]);
    dicParameters=dic.getParameters;
    save(saveFileName,'meanDensityM','expResult','PulsePhase','expRep','condProb','Ref','showData','dicParameters','scriptText');
...
% ------------------------- Experiment sequence ------------------------------------    
    ...
    prog.GenRepeat
        prog.GenIfDo('RegC=',20)
            prog.GenRegOp('RegC=',0);
            prog.GenSeq(Pulse('OnResCooling',0,dic.Tcooling/5));
            prog.GenRegOp('RegA=+1',0);
        prog.GenElseDo
            prog.GenSeq([Pulse('Repump1092',1,9),...
                         Pulse('OpticalPumping',1,7),...
                         Pulse('674PulseShaper',12,dic.piHalfRF-2),...
                         Pulse('RFDDS2Switch',13,dic.piHalfRF)]);
...
%                      Pulse('RFDDS2Switch',1,dic.TimeRF,'phase',-1)]);
    ...
    prog.GenElseEnd;
    % second Ramsey pulse
    prog.GenSeq([Pulse('674PulseShaper',1,dic.piHalfRF-2),...
                 Pulse('RFDDS2Switch',2,dic.piHalfRF,'phase',par)]);    
    ...
    prog.GenSeq([Pulse('Repump1033',0,dic.T1033) Pulse('OffRes422',0,100)]);
    %--------- reference standart Ramsey exp --------------------
    %------------------------------------------------------------
    % initialization
    prog.GenSeq(Pulse('OnResCooling',0,dic.Tcooling));
    prog.GenSeq(Pulse('OpticalPumping',2,7));
    % Ramsey
    prog.GenSeq([Pulse('674PulseShaper',1,dic.piHalfRF-2),...
                 Pulse('RFDDS2Switch',2,dic.piHalfRF,'phase',0)]);
    prog.GenPause(3);
    prog.GenSeq([Pulse('674PulseShaper',1,dic.piHalfRF-2),...
                 Pulse('RFDDS2Switch',2,dic.piHalfRF,'phase',par)]);  
    % detection
