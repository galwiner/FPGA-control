function SideBandFlopWithModulatedBeamNew
...
darkR = zeros(size(PulseTime));

% in MS mode
SBDetuning=1.2;
dic.setNovatech4Freq(0,dic.F674);    
dic.setNovatech4Freq(1,77);  
dic.setNovatech4Freq(2,77);%-SBDetuning);    
...
for index1 = 1:length(PulseTime)
    ...
    dic.setNovatech4Amp(1,1000); dic.setNovatech4Amp(2,0);       
    r=experimentSequence(PulseTime(index1),77);
    dic.setNovatech4Amp(1,0); dic.setNovatech4Amp(2,1000);       
    rR=experimentSequence(PulseTime(index1),77);
...
%     r=experimentSequence(PulseTime(index1),dic.updateF674);
    ...
    if dic.TwoIonFlag
        ...
        darkR(index1) =100-sum( (rR>dic.TwoIonsCountThreshold)*2+...
                             ((rR>dic.darkCountThreshold)&(rR<dic.TwoIonsCountThreshold))*1 ...
                            )/2/length(rR)*100;

    else
        dark(index1) = sum( r<dic.darkCountThreshold)/length(r)*100;
        darkR(index1) = sum( rR<dic.darkCountThreshold)/length(rR)*100;
    end
    AddLinePoint(lines(1),PulseTime(index1),dark(index1));
    AddLinePoint(lines(2),PulseTime(index1),darkR(index1));
    pause(0.1);
end
...
if (dic.AutoSaveFlag)
    ...
    showData='figure;plot(PulseTime,dark,''b'',PulseTime,darkR,''r'');xlabel(''Pulse Time[\mus]'');ylabel(''dark[%]'');';
    saveFileName=fullfile(destDir ,[fileName datestr(now,'-ddmmmyy-HHMMSS')]);
    dicParameters=dic.getParameters;
    save(saveFileName,'PulseTime','dark','darkR','showData','dicParameters','scriptText');
