function MolmerSorensenEchoPhaseEchoTimeScan
...
GatePhase=0:2*pi/20:2*pi;  
EchoStart=0.2:GateTime/20:GateTime;
...
p0=zeros(length(EchoStart),length(GatePhase));
p1=zeros(length(EchoStart),length(GatePhase));
p2=zeros(length(EchoStart),length(GatePhase));
...
if dic.SitOnItFlag
    ...
    for index2 = 1:length(GatePhase)
        ...
        r=experimentSequence(GateTime,Vmodes,EchoStart(index1));
        dic.GUI.sca(1); %get an axis from Dictator GUI to show data
        hist(r,0:1:(1.8*dic.maxPhotonsNumPerReadout));
        if dic.TwoIonFlag
        ...
        dic.GUI.sca(7);
        imagesc(EchoStart,GatePhase,p0);
        axis([min(EchoStart) max(EchoStart) min(GatePhase) max(GatePhase)]);
        colorbar;
        ylabel('GatePhase'); xlabel('EchoStart(mus)'); title('P0');
        dic.GUI.sca(6);
        imagesc(EchoStart,GatePhase,p1);
        axis([min(EchoStart) max(EchoStart) min(GatePhase) max(GatePhase)]);
        colorbar;
        ylabel('GatePhase'); xlabel('EchoStart(mus)'); title('P1');
        dic.GUI.sca(11);
        imagesc(EchoStart,GatePhase,p2);
        axis([min(EchoStart) max(EchoStart) min(GatePhase) max(GatePhase)]);
        colorbar;
        ylabel('GatePhase'); xlabel('EchoStart(mus)'); title('P2');
...
if (dic.AutoSaveFlag)
    ...
    save(saveFileName,'echoduration','EchoStart','GateTime','GatePhase','p0','p1','p2','SBoffset','Vmodes','showData','dicParameters','scriptText');
    disp(['Save data in : ' saveFileName]);
end
%--------------------------------------------------------------------
    function r=experimentSequence(pulsetime,mode,echostart)
        prog=CodeGenerator;
...
%                      Pulse('674DDS1Switch',3,pulsetime,'freq',60)]);   
        ...
        if (pulsetime>echostart+echoduration+1)
