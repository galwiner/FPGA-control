function SingleQubitFrequencyAddressingScan(varargin)
dic=Dictator.me;
savedata=1;
pulseTime=dic.T674;
pulseAmp=100;
...
dark = zeros(length(BeatingFrequency));
fidelity = zeros(length(BeatingFrequency));
...
if dic.SitOnItFlag
    ...
    for index2=1:length(BeatingFrequency)
            if dic.stop
                return;
            end
            if countcheck==10
            ...
            dic.setNovatech('DoublePass','freq',dic.updateF674+MMFreq/2,'amp',1000);
            dic.setNovatech('Blue','freq',BeatingFrequency(index2)/1000,'amp',SxPower);
            dic.setNovatech('Red','freq',dic.SinglePass674freq,'amp',600,'phase',PhaseForSx);
            ...
            AddLinePoint(lines(1),BeatingFrequency(index2),dark(index2));
            AddLinePoint(lines(2),BeatingFrequency(index2),fidelity(index2));
    ...
    showData='figure;plot(BeatingFrequency,fidelity,''b'',BeatingFrequency,dark,''r'');axis([min(BeatingFrequency) max(BeatingFrequency) 0 100]);colorbar;xlabel(''Addressing Frequency (kHz)''); ylabel(''Population''); title(''Single Qubit Addressing Frequency Scan'');';
    dic.save;
...
%                           Pulse('674DoublePass',2,2*PiTimeDDS+pTime+5),... 
%                           Pulse('674Gate',PiTimeDDS+1,pTime),...
%                           Pulse('674DDS1Switch',PiTimeDDS+3,pTime,'phase',phaselastpulse+pi/2,'amp',100),...
%                           Pulse('NoiseEater674',PiTimeDDS+pTime+5,PiTimeDDS-1),...
%                           Pulse('674DDS1Switch',PiTimeDDS+pTime+4,PiTimeDDS,'phase',phaselastpulse,'amp',100)]);
%              prog.GenSeq(Pulse('674PulseShaper',0,1));
% 
