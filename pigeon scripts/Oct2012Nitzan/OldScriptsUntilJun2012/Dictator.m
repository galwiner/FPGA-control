classdef Dictator < handle
    ...
    properties(SetObservable=true) %if one of these properties is
        ...
        Vdcl=-1.77; %FPGA conntrolled
        Vcomp=-10; %FPGA controlled
        AVdcl=0.9; %Agilent controlled (GPIB)
        AVdcr=2.4; %Agilent controlled (GPIB)
        HPVcomp=23.4; %HP controlled (GPIB)
        Vcap=50; %endcap north set voltage, SRS controlled
        Vkeith=1.0; %endcap north set voltage
...
%             end
            ...
            newObj.GUI=PulseGUI(newObj,...
                {'acStarkShift674','F674FWHM','DDSGuageData','DarkMax',...
                'maxPhotonsNumPerReadout', 'noiseEaterDetValue','port2T674',...
                'is674LockedFlag','ULEEStimatorLinewidth','weakOnResAmp',...
                'ULEEstimatorScanNESetPoint','numOfDDS','estimatorDDSNum',...
                'Hp674Freq','stopRun','stp','SitOnEchoFlag','saveIntr',...
                'SMSlaserAlertFlag','LasersLockedFlag','Vcomp','Vdcl'});
...
%                fnov=num2str(f*15/12,9);
%                fprintf(obj.NovatechSerial,['F' num2str(channel) ' ' fnov]);
            ClockFreq=28.63311530666667;
            fr=f*(28.633115306666667/ClockFreq);            
            fprintf(obj.NovatechSerial,['F' num2str(channel) ' ' num2str(fr,10)]);
        end
        ...
        function setNovatech4Clock(obj,p)
            % 0 is internal clock
            if p==0
                fprintf(obj.NovatechSerial,['C I']); 
            else
                fprintf(obj.NovatechSerial,['C E']); 
            end
        end        
        ...
        function handlePropertyEvents(obj,src,evnt)
            ...
            switch src.Name % switch on the property name
                ...
                case 'Vdcl'
                    obj.com.UpdateTrapElectrode(0,0,0,val,obj.Vcomp); pause(1);
                    obj.out(sprintf('Updated DC L electrode to %.2f V',val));
                case 'Vcomp'
                    obj.com.UpdateTrapElectrode(0,0,0,obj.Vdcl,val); pause(1);
