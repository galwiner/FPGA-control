...
%                 else
%                     dic.setNovatech4Amp(1,0)
%                     dic.setNovatech4Amp(0,novatechpower)
%                 end         
            if dic.stop
                return
            end
...
%             dic.setNovatech4Freq(0,Freq674SinglePass+(dic.vibMode(Vmodes(modeInd)).freq+f674Span(index1)+LightShift));
...
%         prog.GenSeq(Pulse('DIO7',0,1));
        ...
        prog.GenSeq([Pulse('NoiseEater674',2,dic.vibMode(mode).coldPiTime),...
                     Pulse('674DDS1Switch',2,dic.vibMode(mode).coldPiTime,'freq',Freq674SinglePass+sb*dic.vibMode(mode).freq+freqdet)]);
        % detection
