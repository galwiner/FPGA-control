...
doheat=0; theat=1000; % in microsec
...
dark = zeros(size(Detuning));
for index1 = 1:length(Detuning)
    if dic.stop
        return
    end
    r=experimentSequence(Detuning(index1),dic.updateF674);
...
if (dic.AutoSaveFlag)
    ...
    showData='figure;plot(Detuning,dark);xlabel(''Sweep Time[\mus]'');ylabel(''dark[%]'');';
    saveFileName=fullfile(destDir ,[fileName datestr(now,'-ddmmmyy-HHMMSS')]);
    dicParameters=dic.getParameters;
    save(saveFileName,'Detuning','dark','showData','dicParameters','scriptText');
    disp(['Save data in : ' saveFileName]);
end 


%--------------------------------------------------------------------
    function r=experimentSequence(detuning,freq)
        prog=CodeGenerator;       
...
%        prog.GenSeq(Pulse('NoiseEater674',1,0,'674DDS1Switch',0,0,'amp',100));
%        prog.GenSeq([Pulse('NoiseEater674',2,0),...
%                         Pulse('674DDS1Switch',0,0,'amp',SweepPower)]);
