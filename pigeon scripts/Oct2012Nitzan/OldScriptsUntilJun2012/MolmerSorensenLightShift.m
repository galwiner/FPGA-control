function MolmerSorensenLightShift
...
% GateDetuning=-0.1:0.005:0.1; 
GateDetuning=0.02:0.001:0.04; 
...
lines =InitializeAxes (dic.GUI.sca(7),...
    'Gate Detuning [MHz]','Populations %','Search for Light Shift',...
...
if dic.SitOnItFlag
    ...
    for index2 = 1:length(GateDetuning)
        dic.setNovatech4Freq(1,Freq674SinglePass-(dic.vibMode(Vmodes(1)).freq-GateDetuning(index2)));
        dic.setNovatech4Freq(0,Freq674SinglePass+(dic.vibMode(Vmodes(1)).freq-GateDetuning(index2)));
