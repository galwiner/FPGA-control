function TwoIonsHeatingWorking20sec(waittime)
dic=Dictator.me;
...
PulseTime=dic.T674;
if ~exist('waittime')
     waittime=[100 1000 5000 10000 20000 30000]; %in msec
end
...
for index2=sind2:iterationsize
    ...
    for index1 = 1:length(waittime)
        ...
        if dic.stop || ~final %||(lengthr~=chunksize)
            savethis;
            return;
        end
    ...
    %     r=experimentSequence(PulseTime(index1),dic.updateF674);
        ...
        if dic.TwoIonFlag
            ...
            pause(0.1);
            fprintf('(%s) Wait= %d // Det Eff=%.2f // Melting =%.2f %%\n',progressStr,waittime(index1),mean(dark(1:index2,index1)),mean(1-initialCrystal(1:index2,index1))*100);
        else
...
%--------------- Save data ------------------
    ...
    function savethis
        ...
        save(saveFileName,'progress','waittime','dark','RepeatedExpt','initialCrystal','finalCrystal','chunksize','repetitions','index1','index2','Vkeith','Vcap','showData','dicParameters','scriptText');
...
%--------------------------------------------------------------------
    function r=experimentSequence(waitt,freq)
        ...
        prog.GenSeq(Pulse('OffRes422',0,1000000)); %one second cooling instead of 500 micoro second
...
%         prog.GenSeq(Pulse('OffRes422',0,20000)); prog.GenSeq(Pulse('OnResCooling',0,dic.Tcooling) );
        prog.GenSeq(Pulse('OpticalPumping',0,dic.Toptpump));
