...
lines =InitializeAxes (dic.GUI.sca(7),...
    ...
    [ShiftSpan(1) ShiftSpan(end)],[0 60],2);
...
for index1 = 1:length(ShiftSpan)
    ...
    r=experimentSequence(ShiftSpan(index1),vibMode);
    if dic.TwoIonFlag
...
%--------------------------------------------------------------------
    function r=experimentSequence(freqShift,mode)
        ...
        % continuous GSC 
        prog.GenSeq([Pulse('674DDS1Switch',1,dic.vibMode(mode).coolingTime,'freq',dic.updateF674+dic.vibMode(mode).freq+freqShift),...
        ...
        prog.GenSeq(Pulse('674DDS1Switch',2,dic.vibMode(mode).hotPiTime,'freq',dic.updateF674+dic.vibMode(mode).freq));
        % detection
