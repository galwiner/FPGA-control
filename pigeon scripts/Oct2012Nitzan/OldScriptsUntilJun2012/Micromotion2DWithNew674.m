...
dark = zeros(length(AVgrid),length(Vcomp));
for index1=1:length(Vcomp)
    for index2 = 1:length(AVgrid)
        dic.AVdcl=AVgrid(index2);
        ...
        dic.GUI.sca(4);
        imagesc(Vcomp,AVgrid,double(dark));
        axis([min(Vcomp) max(Vcomp) min(AVgrid) max(AVgrid)]);
        colorbar;
...
if (dic.AutoSaveFlag)
    ...
    save(saveFileName,'AVgrid','Vcomp','dark','showData','dicParameters','scriptText');
