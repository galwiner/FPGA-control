function MolmerSorensenGateSymAsymDetuning2DScan

dic=Dictator.me;

%  GateDetuning=18:0.3:22; % on Stretch 
GateDetuning=20:0.1:20.1; % on Stretch 
...
if dic.SitOnItFlag
    ...
    for index2 = 1:length(GateDetuning)            
      for index1=1:length(SBoffset)
            ...
            dic.setNovatech('Red','freq',dic.SinglePass674freq+(dic.vibMode(Vmodes).freq-GateDetuning(index2)/1000),'amp',dic.GateInfo.RedAmp);
            ...
            dic.setNovatech('DoublePass','freq',dic.updateF674+SBoffset(index1)/1000/2,'amp',1000);
            ...
            colorbar;
            xlabel('Asymmetric Detuning( kHz)'); ylabel('GateDetuning(mus)'); title('P0');
            dic.GUI.sca(6);
            imagesc(SBoffset,GateDetuning,p1);
            axis([min(SBoffset) max(SBoffset) min(GateDetuning) max(GateDetuning)]);
            colorbar;
            xlabel('Asymmetric Detuning( kHz)'); ylabel('GateDetuning(mus)'); title('P1');
            dic.GUI.sca(11);
            imagesc(SBoffset,GateDetuning,p2);
            axis([min(SBoffset) max(SBoffset) min(GateDetuning) max(GateDetuning)]);
            colorbar;
            xlabel('Asymmetric Detuning( kHz)'); ylabel('GateDetuning(mus)'); title('P2');
            dic.GUI.sca(4);
            imagesc(SBoffset,GateDetuning,abs(p2-p0)+p1);
            axis([min(SBoffset) max(SBoffset) min(GateDetuning) max(GateDetuning)]);
            colorbar;
            xlabel('Asymmetric Detuning( kHz)'); ylabel('GateDetuning(mus)'); title('|P2-P0|+P1');            
...
%------------ Save data ------------------
if (dic.AutoSaveFlag)
    showData=['figure; imagesc(GateDetuning,SBoffset,p0(index1,index2)); axis([min(SBoffset) max(SBoffset) min(GateDetuning) max(GateDetuning)]);colorbar; ylabel(''Asym Detuning ( kHz)''); xlabel(''GateDetuning(mus)''); title(''P0'');' ...
            'figure; imagesc(GateDetuning,SBoffset,p1(index1,index2)); axis([min(SBoffset) max(SBoffset) min(GateDetuning) max(GateDetuning)]);colorbar; ylabel(''Asym Detuning (kHz)''); xlabel(''GateDetuning(mus)''); title(''P1'');' ... 
            'figure; imagesc(GateDetuning,SBoffset,p2(index1,index2)); axis([min(SBoffset) max(SBoffset) min(GateDetuning) max(GateDetuning)]);colorbar; ylabel(''Asym Detuning ( kHz)''); xlabel(''GateDetuning(mus)''); title(''P2'');'];
